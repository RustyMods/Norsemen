...
public partial class VikingAI
{
    ...
    {
        if (m_fleeIfNotAlerted && !HuntPlayer() && m_targetCreature && !IsAlerted() && Vector3.Distance(m_targetCreature.transform.position, transform.position) - (double)m_targetCreature.GetRadius() > m_alertRange)
        {
            Flee(dt, m_targetCreature.transform.position);
            return true;
        }
        
        if (m_fleeIfLowHealth > 0.0 && m_timeSinceHurt < (double)m_fleeTimeSinceHurt && m_targetCreature != null &&
